...
int co_chu_trinh(canh x[], int sl, int ke_tiep){
	int i=0, co_CT=0;
	while (i<sl && !co_CT)
...
void Nhanh_Can(canh a[][size], int n, int sl, int dau, float *TGT, float *CD, float *GNNTT, canh x[], canh PA[]){
	 int j; 	// j la dinh ke tiep 
	 for(j=0; j<n; j++) {
		if (dau!=j && !a[dau][j].da_dung && !co_chu_trinh(x,sl,j)){
			// Ung voi mot gia tri cua j la mot nut tren cay 
		 	// Tinh 2 dai luong cua mot nut trong
	 		
			*TGT = *TGT + a[dau][j].do_dai;
			*CD = can_duoi(a, *TGT, n, sl+1);
			if(*CD < *GNNTT){ 	// Neu CD >= GNNTT thi cat tia 
				// Neu khong co dieu kien nay thi chuong trinh la VET CAN
			  	...
			  	a[j][dau].da_dung=1;
			   	...
			   	if(sl==n-2) { // Da xet het tat ca cac dinh nhung duong di moi co n-1 canh
				   	Cap_Nhat_PA_TNTT(a, n, *TGT, GNNTT, x, PA);
		    	}   	
			 	else
			 		Nhanh_Can(a, n, sl+1, j, TGT, CD, GNNTT, x, PA); //Xet nut con cua nut i
	 		}
