...
void hienthi(){
    ...
    while (window.isOpen())
    {
	    ...
	    if(daCoData == 0){
			sf::Texture whiteBG;
			if (!whiteBG.loadFromFile("./images/white1.png"))
			{
				std::cerr << "load image white1 fail" << std::endl ;
				exit(1);
			}
			sf::Texture loader1;
			if (!loader1.loadFromFile("./images/load1.png"))
			{
				std::cerr << "load image white1 fail" << std::endl ;
				exit(1);
			}
			sf::Texture loader2;
			if (!loader2 .loadFromFile("./images/load2.png"))
			{
			...
			if (!textureGreen.loadFromFile("./images/green.png"))
			{
				std::cerr << "load image green fail" << std::endl ;
				exit(1);
			}
			sf::Texture textureWhite;
			if (!textureWhite.loadFromFile("./images/white2.png"))
			{
				std::cerr << "load image white2 fail" << std::endl ;
				exit(1);
			}
			sf::Texture textureQueeGreen;
			if (!textureQueeGreen.loadFromFile("./images/queen_green.png"))
			{
				std::cerr << "load image queen_green fail" << std::endl ;
				exit(1);
			}
			sf::Texture textureQueenWhite;
			if (!textureQueenWhite.loadFromFile("./images/queen_white.png"))
			{
